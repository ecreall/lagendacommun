...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		var url = static_url+'/Filemanager/scripts/filemanager.config.js';
	} else {
		var url = static_url+'/Filemanager/scripts/filemanager.config.js.default';
	}
...
if($.urlParam('langCode') != 0 && file_exists (static_url+'/Filemanager/scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');

var lg = [];
$.ajax({
  url: static_url+'/Filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
var bindToolbar = function(data) {
	...
	if (!has_capability(data, 'download')) {
		...
		$('#fileinfo').find('button#download').click(function(){
			window.location = data['Path'] //fileConnector + '?mode=download&path=' + encodeURIComponent(data['Path']);
		}).show();
	}
};
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			if(data['Protected']==0) {
				...
				loadJS(static_url+'/Filemanager/scripts/zeroclipboard/copy.js?d' + d.getMilliseconds());
		...
		if(data){
			...
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					result += '<li class="' + cap_classes + '"' + title + '"><div class="clip"><img src="' + data[key]['Preview'] + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '" data-type="' + data[key]['File Type'] + '" data-path="' + data[key]['Path'] + '" /></div><p>' + data[key]['Filename'] + '</p>';
...
$(function(){
	...
	// Loading theme
	loadCSS(static_url+'/Filemanager/themes/' + config.options.theme + '/styles/filemanager.css');
	$.ajax({
	    url:static_url+'/Filemanager/themes/' + config.options.theme + '/styles/ie.css',
	    async: false,
	    success: function(data)
	    {
	        $('head').append(data);
	...
	loadJS(static_url+'/Filemanager/scripts/zeroclipboard/dist/ZeroClipboard.js');
	
	// Loading CodeMirror if enabled for online edition
	if(config.edit.enabled) {
		loadCSS(static_url+'/Filemanager/scripts/CodeMirror/lib/codemirror.css');
		loadCSS(static_url+'/Filemanager/scripts/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS(static_url+'/Filemanager/scripts/CodeMirror/lib/codemirror.js');
		loadJS(static_url+'/Filemanager/scripts/CodeMirror/addon/selection/active-line.js');
		loadJS(static_url+'/Filemanager/scripts/CodeMirror/dynamic-mode.js');
	}
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS(static_url+"/Filemanager/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
	...
	if(config.upload.multiple) {
		...
		loadCSS(static_url+'/Filemanager/scripts/dropzone/downloads/css/dropzone.css');
		loadJS(static_url+'/Filemanager/scripts/dropzone/downloads/dropzone.js');
	...
	if(config.customScrollbar.enabled) {
		loadCSS(static_url+'/Filemanager/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
		loadJS(static_url+'/Filemanager/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');
    ...
    getDetailView(fileRoot + expandedFolder, 'dir');
});
