...
do_buildout() {
    ...
    IMAGE=${IMAGE:-"lagendacommun_lac:latest"}
    # do the buildout
...
case "$1" in
  rebuild)
    mkdir -p cache var volumes/elasticsearch
  ...
  test)
    shift
    docker-compose $options run --rm lac bin/test $@
    ;;
  test_hypatia)
    shift
    docker-compose $options run --rm lac bin/test_hypatia $@
    ;;
  *)
    if [ -z "$1" ]; then
      docker-compose $options up -d
      docker attach lagendacommun_lac_1
    else
